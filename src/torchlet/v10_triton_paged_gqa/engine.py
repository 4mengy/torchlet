...
class Engine:
    def __init__(
        ...
        max_blocks_per_req = _ceil_div(max_seq_len, kv_block_size)
        if max_num_blocks is None:
            # Block 0 is reserved as the empty block-table sentinel.
            max_num_blocks = max_decode_slots * max_blocks_per_req + 1
        ...
        with torch.cuda.device(self.device):
            ...
            with torch.cuda.graph(self.decode_cuda_graph):
                logits = self.model.forward(
                    ...
                    self.decode_forward_params,
                    last_token_only=True,
                )  # [max_decode_slots, vocab_size]
                self.static_gen_tok_id = self.sample(logits)  # [max_decode_slots]
